<script setup lang="ts">
  ...
  onMounted(async () => {  
    ...
    articulo.value.marca.id=0
    articulo.value.marca.nombre=''
    articulo.value.proveedor.id=0
    articulo.value.proveedor.nombre=''
    articulo.value.categorias = []
    categorias_select_id.value =[]
  })

  async function nuevo_articulo(){
    if(!articulo.value.descripcion || !articulo.value.marca.id || !articulo.value.proveedor.id)
      ...
      try {
        ...
        let mensaje = "Ocurrió un error al crear el articulo. Intente nuevamente.";
...
<template>
  ...
  <div class="articulo">
    ...
    <form @submit.prevent="nuevo_articulo" class="formulario">
      ...
      <label class="labelField">Precio:
        <abbr title="Ingrese la descripcion del articulo (obligatorio)">*</abbr>
        <input type="^\d+$" v-model="articulo.precio" placeholder="Debe ingresar el precio del articulo" step="0.01" min="0" pattern="^\d+(\.\d{1,2})?$">
      </label>
      <label class="labelField">Stock:
        <abbr title="Ingrese el stock del articulo (obligatorio)">*</abbr>
        <input type="^\d+$" v-model="articulo.stock" placeholder="Debe ingresar el stock del articulo" pattern="^\d+$">
      </label>
      ...
      <fieldset>
        <input type="submit" class="boton" value="Crear Articulo">
